<template>
  <div id="editor">
    <div v-for="(line, index) in lines" :id="index" :key="'line-'+index" class="row" @click="focus">
      <div :title="index">{{index + 1}}</div>
      <div :title="index">
        <span v-for="(token, i) in line" :key="'line-'+index+'-token-'+i" :title="i" :class="token.type">
          {{token.content}}
        </span>
      </div>
    </div>
    <div id="caret" class="hidden">|</div>
    <button id="run" aria-label="run javascript program" @click="run">></button>
  </div>
</template>
...
#run:hover {
  ...
  box-shadow: 0 0 10px var(--medium-2);
}
